...
public:
    ...
    bool open(){
...
#ifdef LEOPARD_IMX477
        //Set IMX477 camera to work in mode 1 (4K 60 fps)
        ...
        ctrl.value64 = 1;
        if (!xioctl(mFd, VIDIOC_S_EXT_CTRLS, &ctrls))
        {
                qDebug("Failed to configure sensor mode");
                return false;
        }
#endif
        ...
        fmt.fmt.pix.pixelformat = pixfmt;//make_fmt("RG10");//0x30314742;//01GB;
    ...
    void get(void* res){
        ...
        cudaMemcpy(res, mBuffrs[buf.index].start, mWidth * mHeight * sizeof(unsigned short), cudaMemcpyHostToDevice);
...
//        mCamera->get(image);
//        unsigned short* data = (unsigned short*)image.ptr(0);
